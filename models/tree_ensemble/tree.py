...
class Tree(Model):
    ...
    def from_weka(cls, weka_model):
        ...
        if "-B" not in weka_model.to_dict()["options"]:
            raise ValueError(
                "The supplied weka tree was not trained with the '-B' option enabled. Currently we only support binary splits, so please supply '-B' before training your weka model.")
        ...
        def parse_node(subtree, tree):
            ...
            if "!=" in subtree[0]:
                raise ValueError(
                    "Encountered a decision tree with nominal values. This is currently not supported. Please make sure the tree only contains numerical values.")
    ...
    def from_sklearn(cls, sk_model, ensemble_type=None):
        ...
        while (len(node_ids) > 0):
            ...
            if sk_tree.children_left[cur_node] == _tree.TREE_LEAF and sk_tree.children_right[
                cur_node] == _tree.TREE_LEAF:
    ...
    def populate_path_probs(self, node=None, curPath=None, allPaths=None, pathNodes=None, pathLabels=None):
        ...
        # print("Leaf nodes "+str(node.id)+" : "+str(curProb))
            ...
            self.populate_path_probs(node.leftChild, curPath + [node.probLeft], allPaths,
                                     pathNodes + [node.leftChild.id], pathLabels)
            self.populate_path_probs(node.rightChild, curPath + [node.probRight], allPaths,
                                     pathNodes + [node.rightChild.id], pathLabels)
