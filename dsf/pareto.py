...
for d,dataset in enumerate(datasets):
    ...
    if (len(results_list) > 0):
        ...
        for i,j,k in results_list:
            ...
            p_list.append(i)
            acc_list.append(j)
            compression_list.append(k)


        print(acc_list)
        points = pd.DataFrame({"acc": acc_list,
                                   "compression": compression_list})

        mask = paretoset(points, sense=["max", "max"])
        paretoset_points = points[mask]
        print(paretoset_points)
        print(paretoset_points.index)
        print(mask)
        for ind in paretoset_points.index:
            print(p_list[ind])
            thresholds_counter[edge_thresholds.index(p_list[ind])]+=1

        print(thresholds_counter)
