...
stats_file = os.path.join(resultsPath, dataset+'/', 'report_thresholds_compression_'+str(compression_achieved)+'.csv')
        ...
        for i_d,d in enumerate(depths):
            for i_p,p in enumerate(pattern_thresholds):
                ...
                for i_e,e in enumerate(edge_thresholds):
                    ...
                    if (i_e == (len(edge_thresholds) - 1) and abs(1/float(line_arr[14])) <=  compression_achieved):
                        ...
                        if (1/abs(float(line_arr[14])) <=  compression_achieved):
                            index = i_e
                            line_index = lines[counter *(len(edge_thresholds)) * 3  + ((i_e + 1) *3)].split(',')
