...
for size in max_sizes:
    ...
    #writeToReport(report_file, 'Model Size (KB), Nodes Count, Train Accuracy, Test Accuracy, F1 Macro, ROC AUC, Training Time,')
    ...
    for model_type, model_class in learners.items():
                            ...
                            writeToReport(report_file,str(size)+','+str(max_nodes_count) + ','+str(np.round(train_acc,3))+','+str(np.round(test_acc,3))+','+str(np.round(f1_macro,3))+','+str(np.round(roc_auc,3))+','+str(training_time)+',\n')
