...
from models.tree_ensemble.tree import Node, Tree
from models.tree_ensemble.forest import Forest
from models.linear import Linear
from models.model import Model, PredictionType
...
class SSF(Model):
    ...
    @classmethod
    def from_sklearn(cls, forest_model, lr_model, threshold=0.1):
        model = SSF()
    ...
    def get_stumps(cls, forest, threshold, decimal_places):
        ...
        def traverse(cls, node, threshold):
            ...
            if (node.probLeft != None and node.probRight != None):
                if (node.probLeft >= threshold and node.probRight >= threshold): # ssf case
                #if (node.probLeft < threshold or node.probRight < threshold): # less than threshold
                # to select an interval:
                #score = min(node.probLeft, node.probRight)
                #upper = threshold + 0.1
                #if (upper >= score >= threshold):
