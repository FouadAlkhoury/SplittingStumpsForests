...
class DTE:
	...
	def fit(self, train_x, train_y, test_x, test_y):
		...
		extra_features_train_rf, extra_features_train_et  = self.get_extra_features(train_x, train_x, train_y, 0,test_x,test_y, train=True)
		...
		for i in range(1, self.models_max_depth):
			...
			extra_features_train_rf, extra_features_train_et = self.get_extra_features(new_train_x_rf, new_train_x_et, train_y, i,test_x, test_y, train=True)
	...
	def add_layer(self, train_rf_x, train_et_x, train_y):
		rf = RandomForestRegressor(n_estimators=self.n_trees_rf, criterion = "squared_error", max_depth=5, min_samples_leaf=5, random_state=0, max_features = "sqrt", n_jobs=-1)
		et = ExtraTreesRegressor(n_estimators=self.n_trees_et, criterion = "squared_error", max_depth=5, min_samples_leaf=5, random_state=0, max_features = "sqrt", n_jobs=-1)
		
		rf.fit(train_et_x, train_y)
		et.fit(train_rf_x, train_y)

		return (rf,et)
	def get_extra_features(self, x_rf, x_et, y, iteration,test_x,test_y,train):
		...
		if self.tree_embedding_features:
			extra_features_paths_rf, extra_features_paths_et = self.paths_features(x_rf, x_et, y, iteration, train,test_x,test_y)
	...
	def paths_features(self, x_rf, x_et, y, iteration, train,test_x,test_y):
		if train:
			rf = RandomForestRegressor(n_estimators=self.n_trees_rf, criterion = "squared_error", max_depth=5, min_samples_leaf=5, random_state=0, max_features = "sqrt", n_jobs=-1)
			et = ExtraTreesRegressor(n_estimators=self.n_trees_et, criterion = "squared_error", max_depth=5, min_samples_leaf=5, random_state=0, max_features = "sqrt", n_jobs=-1)
			self.model_pca_rf.append(rf)
			self.model_pca_et.append(et)
			rf_decision_path = self.decision_paths(x_rf, y, rf,test_x,test_y)
			et_decision_path = self.decision_paths(x_et, y, et, test_x,test_y)
		...
		return rf,et
	def decision_paths(self, train_x, train_y, model,test_x,test_y):
		kfold = KFold(n_splits=self.n_folds, shuffle=True, random_state = 0)
		...
		for train_index, test_index in kfold.split(valid_x):	
			...
			train_fold_x, train_fold_y = train_x, train_y
