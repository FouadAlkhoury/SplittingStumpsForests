...
class FeatureGeneratingTree(Tree.Tree):
    ...
    def get_features(self, x, output=0):
        ...
        while(curNode.prediction == None):
            counter +=1
            #print(x[curNode.feature])
            #print(curNode.split)
            if (x[curNode.feature] <= curNode.split): 
                weight = np.round(1/(1+math.exp((-1) * x[curNode.feature] + curNode.split)),5)
                curNode = curNode.leftChild
                curNode.weight = weight
                return curNode.weight,1 - curNode.weight
            else:
                weight = np.round((1/(1+math.exp(((-1) * x[curNode.feature]) + curNode.split))),5)
                curNode = curNode.rightChild
                curNode.weight = weight
                return 1 - curNode.weight,curNode.weight
